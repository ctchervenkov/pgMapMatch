...
class mapMatcher():
    ...
    def matchGPXTraceSegment(self, traceSegment):
        ...
        self.clearCurrentRoutes()
        if not (traceSegment.has_times()) and any([pt.time is None for pt in
                                                   traceSegment.points]):  # second is because of bug in gpxpy that doesn't correctly return has_times for short traces
